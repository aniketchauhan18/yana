...
import { userLoginSchema, userRegisterationValidation } from "../validation/user/user.valdiation";
...
export const createUser = async(req: Request, res: Response) => {
  ...
  try {
    ...
    if (emailExits) return res.status(401).json({message: 'email already exists'})
...
export const loginUser = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!email) return res.status(401).json({message: 'Email is not registered'})
    const decodedPassword = await decodePassword(data.password, email.password)
    if (!decodedPassword) return res.status(401).json({message: 'invalid password'})
