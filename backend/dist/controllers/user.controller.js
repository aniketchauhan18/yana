...
const createUser = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const { success, data } = user_valdiation_1.userRegisterationValidation.safeParse(req.body);
    if (!success) {
        return (0, errorResponse_1.InvalidRequestBody)(res);
    }
    console.log(data);
    try {
        const userExists = yield user_model_1.default.findOne({ username: data.username });
        const emailExits = yield user_model_1.default.findOne({ email: data.email });
        if (userExists)
            return res.status(401).json({ message: "username already exists" });
        if (emailExits)
            return res.status(401).json({ message: "email already exists" });
        const hashedPassword = yield (0, hashPassword_1.hashPassword)(data.password);
        const createUser = yield user_model_1.default.create(Object.assign(Object.assign({}, data), { password: hashedPassword }));
...
const loginUser = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { success, data } = user_valdiation_1.userLoginSchema.safeParse(req.body);
    if (!success)
        return (0, errorResponse_1.InvalidRequestBody)(res);
    try {
        ...
        const decodedPassword = yield (0, hashPassword_1.decodePassword)(data.password, email.password);
        if (!decodedPassword)
...
const updateUser = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { success, data, error } = user_valdiation_1.userUpdateValidation.safeParse(req.body);
        console.log(error === null || error === void 0 ? void 0 : error.errors[0].message);
        if (!success)
            return (0, errorResponse_1.InvalidRequestBody)(res);
        const user = yield user_model_1.default.findByIdAndUpdate({ _id: req.params.id }, data, {
            runValidators: true,
        });
        const updatedUser = yield user_model_1.default.findById(user === null || user === void 0 ? void 0 : user._id);
        return res.status(200).json({
